...
def test_returns(request_builder):
    ...
    request_builder.return_type = returns.ReturnType.with_decorator(None, custom)
...
def test_returns_with_multiple_decorators(request_builder, mocker):
    ...
    second_type = request_builder.return_type = returns.ReturnType.with_decorator(
    ...
    assert request_builder.return_type(mock_response) == str(mock_response.json())
...
def test_returns_json(request_builder, mocker):
    ...
    request_builder.return_type = returns.ReturnType.with_decorator(None, returns_json)
...
class TestReturnsJsonCast:
    ...
    def test_with_builtin_type(self, request_builder, mocker):
        ...
        assert request_builder.return_type(mock_response) == str(self.default_value)

    def test_with_builtin_type_and_key(self, request_builder, mocker):
        mock_response = self.prepare_test(request_builder, mocker, key="key", type=int)
        assert request_builder.return_type(mock_response) == 1

...
# Local imports
from uplink import compat, helpers
...
def test_get_arg_spec():
    ...
    signature = compat.get_arg_spec(locals()["func"])
    assert isinstance(signature, compat.Signature)
...
def test_call_args():
    ...
    call_args = compat.get_call_args(func, 1, 2, named=3)
...
class TestURIBuilder(object):
    ...
    def test_variables_not_string(self):
        assert helpers.URIBuilder.variables(None) == set()

    def test_set_variable(self):
        builder = helpers.URIBuilder("/path/to/{variable}")
    ...
    def test_remaining_variables(self):
        builder = helpers.URIBuilder("{variable}")
